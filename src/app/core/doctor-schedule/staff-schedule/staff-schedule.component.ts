...
export class StaffScheduleComponent 
{
 public selectedDate: Date = new Date(2021, 7, 2);
  ...
  public eventSettings: EventSettingsModel = {
    dataSource: extend([], roomData, {}, true) as Record<string, any>[],
    fields: {
      id: 'Id',
  ...
  if (args.type === 'QuickInfo' || args.type === 'Editor' || args.type === 'RecurrenceAlert' || args.type === 'DeleteAlert') {
    const target: Element | undefined = (args.type === 'RecurrenceAlert' ||
      args.type === 'DeleteAlert') ? args.element : args.target;
  ...
  public onRenderCell(args: RenderCellEventArgs): void {
    if (args.element.classList.contains('e-work-cells')) {
      if (typeof args.date !== 'undefined' && args.date< new Date(2021, 6, 31, 0, 0)) {
